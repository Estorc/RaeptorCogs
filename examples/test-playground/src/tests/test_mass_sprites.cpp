...
std::vector<RaeptorCogs::Sprite2D> &loadMassSprites(std::vector<std::string> &fileNames) {
  ...
  std::filesystem::path folderPath = std::string(reinterpret_cast<const char *>(
      RaeptorCogs::LoadFile("protected/folderPath.txt").data())); // change to your folder
  std::filesystem::path jsonFile   = "tags.json";                 // change to your path
  std::vector<std::string> files;

  // Load and parse the JSON file
  try {
    jsonData = nlohmann::json::parse(
        RaeptorCogs::LoadFile((folderPath / jsonFile).string().c_str()));
  ...
  RaeptorCogs::Texture &testTexture =
      RaeptorCogs::ResourceManager<RaeptorCogs::Texture>().create("assets/textures/"
                                                                  "raeptor-cogs-logo."
                                                                  "png");
  for (const auto &item : jsonData.items()) {
    ...
    tex.onLoad = [&tex, &testTexture, key, size = jsonData.size()]() mutable {
      ...
      _sprites.back().setWritingMaskID(static_cast<int>(_sprites.size())); // Unique ID
                                                                           // for each
                                                                           // sprite
...
std::vector<RaeptorCogs::Sprite2D> &
createMassSprites(RaeptorCogs::Texture &texture, size_t count, bool selectable) {
  texture.onLoad = [&texture, count, selectable]() mutable {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      if (selectable) {
        ...
        _sprites.back().setAnchor(glm::vec2(0.5f, 0.5f));
        _sprites.back().setWritingMaskID(static_cast<int>(i) + 1); // Unique ID for each
                                                                   // sprite
        _sprites.back().setReadingMaskID(0);
