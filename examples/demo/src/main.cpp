...
std::string GetFpsString() {
  ...
  fps << std::fixed << std::setprecision(2)
      << 1.0f / RaeptorCogs::Time().getDeltaTime();
  return fps.str();
}
...
void UpdateText() {
  ...
  fps << std::fixed << std::setprecision(2)
      << 1.0f / RaeptorCogs::Time().getDeltaTime();
  raeptorText.setContent(
      "Welcome to RæptorCogs!\nC++ Graphics/Game Engine Framework (" +
      GetFpsString() + " fps)");
}
...
void Initialize() {
  ...
  RaeptorCogs::Font &defaultFont =
      RaeptorCogs::ResourceManager<RaeptorCogs::Font>().get_or_create(
          "assets/fonts/Alef-Bold.ttf");
  defaultFont.onLoad = [&]() {
    ...
    creditsText = RaeptorCogs::Text2D(
        defaultFont,
        "RæptorCogs Demo - © RÆPTOR\nDeveloped by Estorc\n2025 © MIT License");
  ...
  RaeptorCogs::Texture &logoTexture =
      RaeptorCogs::ResourceManager<RaeptorCogs::Texture>().create(
          "assets/textures/raeptor-cogs-logo.png");
  logoTexture.onLoad = [&]() {
    ...
    for (size_t i = 0; i < raeptorParticles.size(); ++i) {
      ...
      raeptorParticles[i].sprite.setColor(
          {RaeptorCogs::Random().getFloat(0.0f, 1.0f),
           RaeptorCogs::Random().getFloat(0.0f, 1.0f),
           RaeptorCogs::Random().getFloat(0.0f, 1.0f)});
      RaeptorCogs::Renderer().add(raeptorParticles[i].sprite);
      raeptorParticles[i].speed =
          50.0f + RaeptorCogs::Random().getFloat(0.0f, 150.0f);
    }
  };
}
...
void Update(RaeptorCogs::Window &mainWindow) {
  ...
  if (lastTime + 1.0f < RaeptorCogs::Time().getTime() &&
      raeptorText.isVisible()) {
  ...
  for (size_t i = 0; i < raeptorParticles.size(); ++i) {
    ...
    if (pos.y > frameSize.y / 2.0f) {
      ...
      pos.x = RaeptorCogs::Random().getFloat(0.0f, frameSize.x) -
              frameSize.x / 2.0f;
    }
    raeptorParticles[i].sprite.setPosition(pos);
    raeptorParticles[i].sprite.setRotation(
        raeptorParticles[i].sprite.getRotation() +
        raeptorParticles[i].speed / 30.0f * RaeptorCogs::Time().getDeltaTime());
  }
...
int main() {
  ...
  Initialize();
  RaeptorCogs::Window &mainWindow = *RaeptorCogs::Platform().createWindow(
      WINDOW_WIDTH, WINDOW_HEIGHT, "RaeptorCogs Demo");
  glfwSwapInterval(0); // Enable VSync
  mainWindow.setIcon({"assets/icons/raeptor-cogs-icon-16.png",
