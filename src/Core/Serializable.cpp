...
std::vector<uint8_t> Serializable::dump(SerializationFlag flags) {
  ...
  case SerializationFlag::COMPRESS: {
    ...
    if (compress(
            out.data(),
            &compressed_size,
            (const Bytef *)ss.str().data(),
            static_cast<uLongf>(ss.tellp())) != Z_OK)
...
void Serializable::load(std::vector<uint8_t> data, SerializationFlag flags) {
  ...
  switch (flags) {
  case SerializationFlag::COMPRESS: {
    out.resize(data.size() * 4); // Start with a buffer 4 times the size of the compressed
                                 // data
    ...
    while (
        uncompress(
            out.data(),
            (uLongf *)&expected_size,
            (const Bytef *)data.data(),
            static_cast<uLongf>(data.size())) != Z_OK) {
  ...
  std::stringstream in(
      std::string((char *)out.data(), out.size()), std::ios::in | std::ios::binary);
