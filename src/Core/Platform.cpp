...
Window *Platform::createWindow(int width, int height, const std::string_view &title) {
  ...
  if (!this->renderer) {
    std::cerr
        << "RendererBackend::createWindow called before renderer is initialized."
        << std::endl;
    return nullptr;
  }
  ...
  windows.push_back(new Window(
      ...
      nullptr,
      frontWindow,
      this->renderer->getBackend().createWindowContext()));
  if (!windows.back()) {
...
void Platform::destroyWindow(Window *window) {
  if (!this->renderer) {
    std::cerr
        << "RendererBackend::destroyWindow called before renderer is initialized."
        << std::endl;
    return;
  }
  if (!window) {
