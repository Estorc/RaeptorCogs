...
std::shared_ptr<FontBase>
FontBase::create(const FileData &ttf_buffer, FontOptions options) {
  ...
  RaeptorCogs::ResourceWorker().addJob(
      [font, ttf_buffer, priority]() {
        ...
        RaeptorCogs::MainWorker().addJob(
            [font, data, atlas_width, atlas_height]() {
...
std::shared_ptr<FontBase>
FontBase::create(const std::filesystem::path &fontPath, FontOptions options) {
...
FontBase::~FontBase() {
  ...
  for (auto &glyph : glyphs) {
    if (glyph.second) glyph.second.reset(); // Clean up GlyphData pointers
  }
  // glDeleteTextures(1, &textureID);
}

void FontBase::GenerateSDFAtlas(
    const unsigned char *ttf_buffer, unsigned char *data, size_t atlas_w, size_t atlas_h,
  ...
  for (unsigned int cp = 0; cp < num_chars; ++cp) { // skip control chars
    ...
    unsigned char *sdf = stbtt_GetCodepointSDF(
        &font, scale, static_cast<int>(cp), 8, 128, 64.0f, &w, &h, &xoff, &yoff);
    ...
    for (size_t row = 0; row < static_cast<size_t>(h); ++row) {
      memcpy(
          data + (static_cast<size_t>(rect.y) + row) * atlas_w +
              static_cast<size_t>(rect.x),
          sdf + row * static_cast<size_t>(w),
...
void FontBase::GenerateBMPAtlas(
    const unsigned char *ttf_buffer, unsigned char *data, size_t atlas_w, size_t atlas_h,
    float font_px, size_t num_chars) {
  (void)font_px;
  stbtt_pack_context ctx;
  if (stbtt_PackBegin(
          &ctx, data, static_cast<int>(atlas_w), static_cast<int>(atlas_h), 0, 1, NULL) ==
      0) {
