...
extern "C" void file_callback_bridge(
    const unsigned char *data, int size, int callbackID, const char *name) {
...
EM_JS(void, js_open_file_dialog, (int callbackID), {
  ...
  input.onchange = function(event) {
    ...
    reader.onload = function(e) {
      ...
      var buf = Module.HEAPU8.subarray(
          Module._malloc(data.length), Module._malloc(data.length) + data.length);
      buf.set(data);
      Module.ccall(
          'file_callback_bridge',
          null,
          [ 'number', 'number', 'number', 'string' ],
          [ buf.byteOffset, data.length, callbackID, name ]);
