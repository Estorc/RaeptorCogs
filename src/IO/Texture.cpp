...
TextureAtlas::TextureAtlas()
    : TextureAtlas(glm::ivec2(COMMON_ATLAS_SIZE, COMMON_ATLAS_SIZE)) {}
TextureAtlas::TextureAtlas(glm::ivec2 size, GLenum minFilter, GLenum magFilter)
    : flags(TextureAtlasFlags::NONE), minFilter(minFilter), magFilter(magFilter) {
#ifndef NDEBUG
  std::cout
      << "Creating TextureAtlas with size: " << size.x << "x" << size.y << std::endl;
#endif
  this->size      = size;
  this->freeSpace = size.x * size.y;
  this->nodes.resize(static_cast<size_t>(this->size.x));
  stbrp_init_target(
      &this->ctx, this->size.x, this->size.y, this->nodes.data(), this->size.x);
}
...
void TextureAtlas::uploadTexture(
    GLint x, GLint y, GLint width, GLint height, const void *data, bool newAtlas) {
  if (newAtlas) {
    this->glTexture->build(
        this->size.x, this->size.y, nullptr, this->minFilter, this->magFilter);
  }
...
void TextureAtlas::removeTexture(const TextureBase &texture) {
  ...
  glFramebufferTexture2D(
      GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this->getID(), 0);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo->getID());
  glFramebufferTexture2D(
      GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, newTexture->getID(), 0);
  ...
  for (size_t i = 0; i < this->textures.size(); ++i) {
    ...
    glBlitFramebuffer(
        static_cast<GLint>(oldRect.x),
        static_cast<GLint>(oldRect.y),
        static_cast<GLint>(oldRect.x + oldRect.z),
        static_cast<GLint>(oldRect.y + oldRect.w),
        static_cast<GLint>(newRect.x),
        static_cast<GLint>(newRect.y),
        static_cast<GLint>(newRect.x + newRect.z),
        static_cast<GLint>(newRect.y + newRect.w),
        GL_COLOR_BUFFER_BIT,
        GL_NEAREST);
  }
  glBindFramebuffer(GL_READ_FRAMEBUFFER, fbo->getID());
  glFramebufferTexture2D(
      GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, newTexture->getID(), 0);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo->getID());
  glFramebufferTexture2D(
      GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, this->getID(), 0);
  glDrawBuffers(2, drawBuffers);
  glBlitFramebuffer(
      0,
      0,
...
void TextureBase::upload(const Image &img) {
  ...
  std::shared_ptr<TextureAtlas> atlas =
      RaeptorCogs::Renderer().getTextureAtlasManager().getAtlas(
          std::make_tuple(GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR));
  uint64_t allocatedSize = NextPowerOf2(
      std::max(img.width + ATLAS_PADDING * 2, img.height + ATLAS_PADDING * 2));
  ...
  if (!needsNewAtlas) {
    needsNewAtlas = !atlas->tryAddTexture(
        this, static_cast<int>(img.width), static_cast<int>(img.height));
    textureAdded = !needsNewAtlas;
  }
  if (needsNewAtlas)
    atlas = std::make_shared<TextureAtlas>(
        glm::ivec2(allocatedSize, allocatedSize), GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR);
  if (textureAdded ||
      atlas->tryAddTexture(
          this, static_cast<int>(img.width), static_cast<int>(img.height))) {
    atlas->uploadTexture(
        static_cast<GLint>(this->rect.x),
        static_cast<GLint>(this->rect.y),
        static_cast<GLint>(this->rect.z),
        static_cast<GLint>(this->rect.w),
        img.data.get(),
        needsNewAtlas);
...
std::shared_ptr<TextureBase>
TextureBase::create(const FileData &fileData, TextureOptions options) {
  auto texture = std::shared_ptr<TextureBase>(new TextureBase());
  RaeptorCogs::ResourceWorker().addJob(
      [self = texture, fileData, options]() {
        auto img = std::make_shared<Image>(
            LoadImageFromMemory(fileData, options.s_width, options.s_height));
        if (!img->data) {
        ...
        RaeptorCogs::MainWorker().addJob(
            [self, img]() {
              if (self.use_count() <= 2)
                return; // If no one else is using this texture, skip uploading
              self->upload(*img);
...
std::shared_ptr<TextureBase>
TextureBase::create(const std::filesystem::path &filepath, TextureOptions options) {
  auto texture = std::shared_ptr<TextureBase>(new TextureBase());
  if (filepath.empty()) {
  ...
  RaeptorCogs::ResourceWorker().addJob(
      [self = texture, str = filepath.string(), options]() {
        auto img = std::make_shared<Image>(
            LoadImageFromFile(str.c_str(), options.s_width, options.s_height));
        if (!img->data) {
          std::cerr << "Failed to create texture from image: No data." << std::endl;
          return;
        }
...
std::shared_ptr<TextureBase>
TextureBase::create(unsigned int width, unsigned int height, TextureOptions options) {
  auto texture = std::shared_ptr<TextureBase>(new TextureBase());
  RaeptorCogs::ResourceWorker().addJob(
      [self = texture, width, height, options]() {
...
#ifndef NDEBUG
  std::cout
      << "Added atlas. Total atlases of this type: " << this->atlases[key].size()
      << std::endl;
#endif
}
...
void TextureAtlasManager::sort(TextureAtlasTypeKey key) {
  ...
  std::sort(
      it->second.begin(),
      it->second.end(),
      [](const std::shared_ptr<TextureAtlas> &a, const std::shared_ptr<TextureAtlas> &b) {
