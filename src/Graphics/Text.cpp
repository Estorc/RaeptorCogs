...
Glyph::Glyph(Text2D &text, const U8Char &character, glm::vec2 advance)
...
void Glyph::setCharacter(const U8Char &_character, glm::vec2 advance) {
  ...
  this->setSize(
      this->text->getFont()->getGlyphSize(_character) *
      (this->text->getTextSize() / NORMAL_FONT_SIZE));
  ...
  glm::vec2 glyphOffset =
      this->text->getFont()->getGlyphOffset(_character) *
      (this->text->getTextSize() / NORMAL_FONT_SIZE);

  this->setPosition(this->getPosition() + glyphOffset * 2.0f + advance);
}

bool Glyph::computeInstanceData(
    GAPI::Common::InstanceAllocator &instanceAllocator, ComputeInstanceDataMode mode) {
  ...
  auto &staticDataBuffer =
      instanceAllocator.getStaticInstanceData(batchHandler.staticDataCursor);
  auto *dynamicDataBuffer =
      instanceAllocator.getDynamicInstanceData(batchHandler.dynamicDataCursor);
  ...
  if (this->isDataDirty() || mode == ComputeInstanceDataMode::FORCE_REBUILD) {
    ...
    staticDataBuffer.type =
        this->isVisible()
            ? RendererMode::RENDERER_MODE_2D_TEXT
            : RendererMode::RENDERER_MODE_DEFAULT;
    staticDataBuffer.readingMaskID = this->getReadingMaskID();
    staticDataBuffer.writingMaskID = this->getWritingMaskID();
    if (mode == ComputeInstanceDataMode::FORCE_REBUILD) {
      staticDataBuffer.dataOffset = batchHandler.dynamicDataCursor; // Offset into the
                                                                    // instance data
                                                                    // buffer
    }
  ...
  if (font) {
    ...
    for (auto it = content.begin(); it != content.end(); ++it, ++charCount) {
      ...
      if (glyphs.size() < charCount + 1) {
        glyphs.push_back(std::make_shared<Glyph>(
            *this,
            c,
            advance -
                glm::vec2(size.x - alignOffset * 2.0f, size.y) * this->getAnchor()));
        this->getRenderer()->add(*glyphs[charCount].get());
      } else {
        glyphs[charCount]->setCharacter(
            c,
            advance - glm::vec2(size.x - alignOffset * 2.0f, size.y) * this->getAnchor());
      }
      if (c == "\n") {
      ...
      // advance.x += font->getGlyphSize(reinterpret_cast<const unsigned
      // char*>(&content[i])).x / 2.0f; // Add the x offset of the glyph

      if (this->wordWrapType == TextWordWrap::CHARACTER &&
          advance.x > this->wordWrapWidth) {
        // If character wrapping is enabled and the width exceeds the limit, move to the
        // next line
        advance.x = 0.0f;        // Reset x offset for new line
        advance.y += lineHeight; // Move down by line height
      ...
      if (this->wordWrapType == TextWordWrap::WORD && advance.x > this->wordWrapWidth &&
          lastSpaceIt != content.end()) {
        // If word wrapping is enabled and the width exceeds the limit, move to the next
        // line
    ...
    glyphs.resize(charCount); // Resize glyphs vector to match the number of characters
                              // processed
  } else {
...
glm::vec2 Text2D::measureTextSize() const {
  ...
  for (auto it = content.begin(); it != content.end(); ++it) {
    ...
    if (this->wordWrapType == TextWordWrap::CHARACTER &&
        currentLineWidth > this->wordWrapWidth) {
      // If character wrapping is enabled and the width exceeds the limit, move to the
      // next line
    ...
    if (this->wordWrapType == TextWordWrap::WORD &&
        currentLineWidth > this->wordWrapWidth && lastSpaceIt != content.end()) {
      // If word wrapping is enabled and the width exceeds the limit, move to the next
      // line
...
float Text2D::measureLineWidth(const U8String &text) const {
  ...
  for (auto c : text) {
    ...
    if (this->wordWrapType == TextWordWrap::WORD && width > this->wordWrapWidth &&
        lastSpaceWidth != 0.0f) {
      // If word wrapping is enabled and the width exceeds the limit, move to the next
      // line
