...
void ShaderStorageBuffer::allocate(size_t size) {
  ...
  size_t texHeight =
      (size + (texWidth * BYTES_PER_VEC4 - 1)) / (texWidth * BYTES_PER_VEC4);
  ...
  std::cout
      << "Allocated Shader Storage Buffer of size " << size << " bytes (" << texWidth
      << "x" << texHeight << " texture)." << std::endl;
}
