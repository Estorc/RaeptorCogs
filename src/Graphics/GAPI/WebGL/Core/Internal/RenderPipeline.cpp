...
void RenderPipeline::endFrame() {
  GL::RenderPipeline::endFrame();
}

void RenderPipeline::renderPass(int x, int y, int width, int height) {
  GraphicCore &graphicCore =
      static_cast<GraphicCore &>(this->getRenderer().getGraphicCore());
...
void RenderPipeline::renderMask(Window *window, int x, int y, int width, int height) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    glFramebufferRenderbuffer(
        ...
        GL_RENDERBUFFER,
        graphicCore.getRenderbuffer()->getID());
    glFramebufferTexture2D(
        GL_FRAMEBUFFER,
        GL_COLOR_ATTACHMENT0,
        GL_TEXTURE_2D,
        graphicCore.getMaskTextures().first->getID(),
        0);
    ...
    std::swap(
        graphicCore.getPingPongMaskFramebuffer().first,
        graphicCore.getPingPongMaskFramebuffer().second);
  }
  ...
  this->processBatch([x, y, width, height, &graphicCore]() {
    glBindFramebuffer(
        GL_READ_FRAMEBUFFER, graphicCore.getPingPongMaskFramebuffer().second->getID());
    glBindFramebuffer(
        GL_DRAW_FRAMEBUFFER, graphicCore.getPingPongMaskFramebuffer().first->getID());
    glBlitFramebuffer(
        x,
        y,
        x + width,
