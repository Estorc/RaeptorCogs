...
void GraphicCore::initGraphics() {
    ...
    Window* window = new Window(800, 600, "", nullptr, nullptr, new WebGL::WindowContext(&this->getRenderer()), WindowFlags::CONTEXT_ONLY | WindowFlags::DESTROY_AFTER_INIT);
    if (!window) {
...
void GraphicCore::buildShaders() {
    std::vector<rgsl_shader_blob> shaders(rgsl_shaders, rgsl_shaders + sizeof(rgsl_shaders) / sizeof(rgsl_shader_blob));
    Common::ShaderInfo shaderInfo = {};
    shaderInfo.vertex_shader = FindEmbeddedShader(shaders, "shader_main", RGSL_VERTEX, "es");
    shaderInfo.fragment_shader = FindEmbeddedShader(shaders, "shader_main", RGSL_FRAGMENT, "es");
    this->getMainShader()->build(shaderInfo);
    shaderInfo.fragment_shader = FindEmbeddedShader(shaders, "shader_mask", RGSL_FRAGMENT, "es");
...
void GraphicCore::drawElementsInstancedBaseVertexBaseInstance(size_t count, size_t instanceCount, size_t first, int baseVertex, unsigned int baseInstance) {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(count), GL_UNSIGNED_INT, (void*)(0 * sizeof(GLuint)), static_cast<GLsizei>(instanceCount));
