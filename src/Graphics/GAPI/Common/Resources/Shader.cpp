...
const void * FindEmbeddedShader(const void* rgsl_shaders, size_t shader_size, size_t shader_count, const std::string& name, ShaderStage stage, const std::vector<std::pair<ShaderStage, size_t>>& stages_map, const std::string& profile) {
    for (size_t i = 0; i < shader_count; i++) {
        const rgsl_shader_blob *shader = reinterpret_cast<const rgsl_shader_blob *>(static_cast<const char*>(rgsl_shaders) + i * shader_size);
        ...
        if (name == shader->name && mapped_stage == shader->stage && profile == shader->profile) {
            return shader;
        }
    }
    std::cerr << "Warning: Shader not found - Name: " << name << ", Stage: " << static_cast<size_t>(stage) << ", Profile: " << profile << std::endl;
    return nullptr;
}
