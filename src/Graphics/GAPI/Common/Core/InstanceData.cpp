...
void InstanceAllocator::allocate(
    GraphicBatchHandler &batchHandler, size_t dynamicDataSize) {
  ...
  batchHandler.dynamicDataCursor =
      static_cast<unsigned int>(this->allocateDynamicData(dynamicDataSize));
}
...
void InstanceUploader::upload(
  ...
  for (const auto &region : this->dirtyStaticDataRegions) {
    ...
    staticInstanceDataSSBO->get()->upload(
        this->instanceData.getStatic().data() + begin,
        (end - begin) * sizeof(Common::StaticInstanceData),
        begin * sizeof(Common::StaticInstanceData));
  }
  for (const auto &region : this->dirtyDynamicDataRegions) {
    size_t begin = region.first;
    size_t end   = region.second;
    dynamicInstanceDataSSBO->get()->bind();
    dynamicInstanceDataSSBO->get()->upload(
        this->instanceData.getDynamic().data() + begin,
        (end - begin) * sizeof(Common::DynamicInstanceData),
        begin * sizeof(Common::DynamicInstanceData));
  }
