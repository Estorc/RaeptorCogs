...
void GraphicCore::initGraphics() {
  ...
  Window *window = new Window(
      ...
      nullptr,
      nullptr,
      new GL::WindowContext(&this->getRenderer()),
...
void GraphicCore::buildShaders() {
  ...
  shaderInfo.vertex_shader =
      FindEmbeddedShader("shader_main", Common::ShaderStage::VERTEX, "core");
  shaderInfo.fragment_shader =
      FindEmbeddedShader("shader_main", Common::ShaderStage::FRAGMENT, "core");
  this->getMainShader()->build(shaderInfo);
  shaderInfo.fragment_shader =
      FindEmbeddedShader("shader_mask", Common::ShaderStage::FRAGMENT, "core");
...
void GraphicCore::buildBuffers() {
  ...
  for (int i = 0; i <= CIRCLE_SEGMENTS; ++i) {
    float angle =
        static_cast<float>(i) * 2.0f * glm::pi<float>() /
  ...
  shapesVertices.insert(
      shapesVertices.end(), circleVertices.begin(), circleVertices.end());
  ...
  this->windows.front()->getWidth(), this->windows.front()->getHeight());
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      static_cast<GLsizeiptr>(sizeof(Vertex2D) * shapesVertices.size()),
      shapesVertices.data(),
      GL_STATIC_DRAW);
  ...
  glBufferData(
      ...
      static_cast<GLsizeiptr>(sizeof(unsigned int) * shapesIndices.size()),
      shapesIndices.data(),
      GL_STATIC_DRAW);
}
