...
GAPI::ObjectHandler<GAPI::Common::VertexArray> *
WindowContext::getQuadVertexArray() {
  return &this->quadVertexArray;
}

void WindowContext::buildVAO() {
  RendererBackend &backend =
      static_cast<RendererBackend &>(*this->getRenderer());
  if (!backend.getGraphicCore().getQuadVBO().get())
    return;
  ...
  backend.getGraphicCore().getStaticInstanceDataSSBO()->allocate(
      Common::STATIC_INSTANCE_DATA_SIZE);
  backend.getGraphicCore().getStaticInstanceDataSSBO()->setBindingPoint(1);
  backend.getGraphicCore().getDynamicInstanceDataSSBO()->bind();
  backend.getGraphicCore().getDynamicInstanceDataSSBO()->allocate(
      Common::DEFAULT_DYNAMIC_DATA_BUFFER_SIZE);
