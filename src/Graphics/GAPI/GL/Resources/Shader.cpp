...
void Shader::build(const Common::ShaderInfo &shader) {
  ...
  rgsl_shader_blob vertexBlob =
      *static_cast<const rgsl_shader_blob *>(shader.vertex_shader);
  rgsl_shader_blob fragmentBlob =
      *static_cast<const rgsl_shader_blob *>(shader.fragment_shader);
  ...
  this->build(
      static_cast<const char *>(vertexBlob.glsl_code),
      static_cast<const char *>(fragmentBlob.glsl_code));
}
...
void Shader::setMat2(const std::string &name, const glm::mat2 &mat) const {
  glUniformMatrix2fv(
      glGetUniformLocation(this->getID(), name.c_str()), 1, GL_FALSE, &mat[0][0]);
}
