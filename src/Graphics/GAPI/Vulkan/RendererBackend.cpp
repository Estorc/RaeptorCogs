...
void RendererBackend::setupDebugMessenger() {
    ...
    auto func = (PFN_vkCreateDebugUtilsMessengerEXT)
        vkGetInstanceProcAddr(this->getGraphicCore().getInstance(), "vkCreateDebugUtilsMessengerEXT");
    if (func) {
        std::cout << "Setting up Vulkan debug messenger." << std::endl;
        func(this->getGraphicCore().getInstance(), &debugCreateInfo, nullptr, &debugMessenger);
    } else {
...
void RendererBackend::pickPhysicalDevice() {
    ...
    vkEnumeratePhysicalDevices(this->getGraphicCore().getInstance(), &physicalDeviceCount, NULL);
    if (physicalDeviceCount == 0) {
        std::cerr << "Failed to find GPUs with Vulkan support!" << std::endl;
        return;
    }
    std::vector<VkPhysicalDevice> physicalDevices(physicalDeviceCount);
    vkEnumeratePhysicalDevices(this->getGraphicCore().getInstance(), &physicalDeviceCount, physicalDevices.data());
    ...
    if (this->getGraphicCore().getPhysicalDevice() == VK_NULL_HANDLE) {
        ...
        vkGetPhysicalDeviceProperties(this->getGraphicCore().getPhysicalDevice(), &deviceProperties);
...
void RendererBackend::createQueueAndLogicalDevice() {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(this->getGraphicCore().getPhysicalDevice(), &queueFamilyCount, nullptr);

    std::vector<VkQueueFamilyProperties> queueFamilies(queueFamilyCount);
    vkGetPhysicalDeviceQueueFamilyProperties(this->getGraphicCore().getPhysicalDevice(), &queueFamilyCount, queueFamilies.data());
    ...
    for (uint32_t i = 0; i < queueFamilyCount; i++) {
        if (queueFamilies[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) {
            ...
            queueCreateInfo.queueFamilyIndex = this->getGraphicCore().getGraphicsQueueFamilyIndex();
            ...
            vkCreateDevice(this->getGraphicCore().getPhysicalDevice(), &deviceCreateInfo, nullptr, &this->getGraphicCore().getDevice());
            vkGetDeviceQueue(this->getGraphicCore().getDevice(), i, 0, &this->getGraphicCore().getGraphicsQueue());
            vkGetDeviceQueue(this->getGraphicCore().getDevice(), i, 0, &this->getGraphicCore().getPresentQueue());
            break;
        }
    }
}
...
void RendererBackend::createPipelineCache() {
    ...
    vkCreatePipelineCache(this->getGraphicCore().getDevice(), &cacheInfo, nullptr, &this->getGraphicCore().getPipelineCache());
}
...
void RendererBackend::createCommandPool() {
    ...
    if (vkCreateCommandPool(this->getGraphicCore().getDevice(), &poolInfo, nullptr, &this->getGraphicCore().getCommandPool()) != VK_SUCCESS) {
...
void RendererBackend::createDescriptorPool() {
    ...
    vkCreateDescriptorPool(this->getGraphicCore().getDevice(), &pool_info, nullptr, &this->getImGuiModule().getImGuiDescriptorPool());
}
...
void RendererBackend::initializePipeline() {
    ...
    shaderInfo.vertex_shader = FindEmbeddedShader("shader_example", Common::ShaderStage::VERTEX, "core");
    shaderInfo.fragment_shader = FindEmbeddedShader("shader_example", Common::ShaderStage::FRAGMENT, "core");
    ...
    this->getGraphicCore().getQuadVBO()->build(quadVertices.data(), sizeof(RaeptorCogs::Vertex2D) * quadVertices.size());
    this->getGraphicCore().getQuadEBO()->build(indices, sizeof(indices));
}

RendererBackend::~RendererBackend() {
    auto func = (PFN_vkDestroyDebugUtilsMessengerEXT)
        vkGetInstanceProcAddr(this->getGraphicCore().getInstance(), "vkDestroyDebugUtilsMessengerEXT");
    if (func) {
    ...
    //vkFreeMemory(this->getGraphicCore().getDevice(), this->getGraphicCore().getStagingBufferMemory(), nullptr);

    vkDestroyCommandPool(this->getGraphicCore().getDevice(), this->getGraphicCore().getCommandPool(), nullptr);
    this->getObjectAllocator().destroyAll();
    vkDestroyPipelineCache(this->getGraphicCore().getDevice(), this->getGraphicCore().getPipelineCache(), nullptr);
    vkDestroyDescriptorPool(this->getGraphicCore().getDevice(), this->getImGuiModule().getImGuiDescriptorPool(), nullptr);
