...
void Shader::build(const Common::ShaderInfo &shaderInfo) {
  ...
  RendererBackend &vkRenderer =
      static_cast<RendererBackend &>(RaeptorCogs::Renderer().getBackend());
  VkDevice device    = vkRenderer.getGraphicCore().getDevice();
  WindowContext &ctx = *static_cast<WindowContext *>(
      RaeptorCogs::Platform().getWindows().front()->getContext());
  ...
  VkShaderModule vertModule =
      CreateShaderModule(device, vertex_blob->spirv_words, vertex_blob->word_count);
  VkShaderModule fragModule =
      CreateShaderModule(device, fragment_blob->spirv_words, fragment_blob->word_count);
  ...
  vkCreateDescriptorSetLayout(device, &setLayoutInfo, NULL, &this->descriptorSetLayout);

  VkPipelineLayoutCreateInfo layoutInfo = {};
  layoutInfo.sType                      = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
  layoutInfo.setLayoutCount             = 1;
  layoutInfo.pSetLayouts                = &this->descriptorSetLayout;
  if (vkCreatePipelineLayout(device, &layoutInfo, NULL, &this->pipelineLayout) !=
      VK_SUCCESS) {
    std::cerr
        << "ERROR::VULKAN_SHADER::BUILD::PIPELINE_LAYOUT_CREATION_FAILED" << std::endl;
    return;
  }
  ...
  pipelineRendering.depthAttachmentFormat   = VK_FORMAT_D32_SFLOAT; // or
                                                                    // VK_FORMAT_UNDEFINED
  ...
  if (vkCreateGraphicsPipelines(
          device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &this->pipeline) !=
      VK_SUCCESS) {
    std::cerr
        << "ERROR::VULKAN_SHADER::BUILD::GRAPHICS_PIPELINE_CREATION_FAILED" << std::endl;
    return;
  }
...
VkPipeline Shader::getPipeline() const {
...
VkDescriptorSetLayout Shader::getDescriptorSetLayout() const {
...
const void *FindEmbeddedShader(
  ...
  return static_cast<const void *>(Common::FindEmbeddedShader(
      rgsl_shaders,
      sizeof(rgsl_shaders[0]),
      sizeof(rgsl_shaders) / sizeof(rgsl_shaders[0]),
      name,
      stage,
      STAGES_MAP,
      profile));
