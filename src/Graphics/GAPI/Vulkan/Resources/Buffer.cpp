...
void Buffer::vkBuild(const void *data, size_t size, VkBufferUsageFlags usage) {
  ...
  VkDeviceMemory stagingBufferMemory = AllocateMemory<VkBuffer>(
      &backend,
      stagingBuffer,
  ...
  this->memory = AllocateMemory<VkBuffer>(
      &backend, this->buffer, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void Vertexbuffer::bind() {
  ...
  vkCmdBindVertexBuffers(
      backend.getRenderPipeline().getVulkanFrame().commandBuffer,
      0,
      1,
      &this->getBuffer(),
      offsets);
