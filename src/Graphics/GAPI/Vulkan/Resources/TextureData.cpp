...
void TextureData::build(int width, int height, void * data, GLenum minFilter, GLenum magFilter) {
    RendererBackend& renderer = static_cast<RendererBackend&>(RaeptorCogs::Renderer().getBackend());
    int mipLevels = static_cast<int>(std::floor(std::log2(std::max(width, height)))) + 1; // Calculate number of mipmap levels
    ...
    this->memory = AllocateMemory<VkImage>(&renderer, this->image, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    vkCreateBuffer(renderer.getGraphicCore().getDevice(), &bufferInfo, nullptr, &stagingBuffer);
    stagingBufferMemory = AllocateMemory<VkBuffer>(&renderer, stagingBuffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    ...
    vkCreateImageView(renderer.getGraphicCore().getDevice(), &viewInfo, NULL, &this->textureView);
    ...
    vkCreateSampler(renderer.getGraphicCore().getDevice(), &samplerInfo, NULL, &this->textureSampler);


}

void TextureData::bind() {
    VkDevice device = static_cast<RendererBackend&>(RaeptorCogs::Renderer().getBackend()).getGraphicCore().getDevice();
