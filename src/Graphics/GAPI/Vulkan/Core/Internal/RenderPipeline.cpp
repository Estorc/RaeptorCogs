...
void RenderPipeline::transitionImageLayout(
...
void beginDraw(
    RendererBackend *backend, WindowContext *, VkCommandBuffer commandBuffer,
...
void endDraw(
    GraphicCore &graphicCore, VkCommandBuffer commandBuffer,
    WindowContext *currentContext, uint32_t *imageIndex) {
  VkSemaphore imageAvailableSemaphore =
      currentContext->getImageAvailableSemaphore(currentContext->getCurrentFrame());
  VkSemaphore renderFinishedSemaphore =
      currentContext->getRenderFinishedSemaphore(currentContext->getCurrentFrame());
  VkFence inFlightFence =
      currentContext->getInFlightFence(currentContext->getCurrentFrame());
  ...
  if (vkQueueSubmit(graphicCore.getGraphicsQueue(), 1, &submitInfo, inFlightFence) !=
      VK_SUCCESS) {
...
void acquireNextImage(
    WindowContext *currentContext, VkDevice device, uint32_t *imageIndex) {
  VkSemaphore imageAvailableSemaphore =
      currentContext->getImageAvailableSemaphore(currentContext->getCurrentFrame());
  VkFence inFlightFence =
      currentContext->getInFlightFence(currentContext->getCurrentFrame());
  ...
  if (currentContext->getImageInFlight(*imageIndex) != VK_NULL_HANDLE) {
    vkWaitForFences(
        device, 1, &currentContext->getImageInFlight(*imageIndex), VK_TRUE, UINT64_MAX);
  }
...
// Create global Descriptor Set to test descriptor set management
VkDescriptorSet globalDescriptorSet = VK_NULL_HANDLE;
ObjectHandler<Common::TextureData> testTextureData;

void RenderPipeline::renderPass(int, int, int, int) {
  Vulkan::GraphicCore &graphicCore =
      static_cast<Vulkan::GraphicCore &>(this->getRenderer().getGraphicCore());
  Vulkan::RendererBackend &renderer =
      static_cast<Vulkan::RendererBackend &>(this->getRenderer());
  ...
  if (globalDescriptorSet == VK_NULL_HANDLE) {
    std::cout << "Setting up global descriptor set for testing..." << std::endl;
    VkDescriptorPool &descriptorPool =
        static_cast<Vulkan::ImGuiModule &>(getRenderer().getImGuiModule())
            .getImGuiDescriptorPool();
    VkDescriptorSetLayout descriptorSetLayout =
        static_cast<Vulkan::Shader *>(graphicCore.getMainShader().get())
    ...
    if (vkAllocateDescriptorSets(
            graphicCore.getDevice(), &allocInfo, &globalDescriptorSet) != VK_SUCCESS) {
      throw std::runtime_error("failed to allocate descriptor sets!");
    }

    Image testImage = LoadImageFromFile("assets/textures/raeptor-cogs-logo.png");
    std::cout
        << "Loaded test image: " << testImage.width << "x" << testImage.height
        << std::endl;
    testTextureData->build(testImage.width, testImage.height, testImage.data.get());

    VkDescriptorImageInfo imageInfo {};
    imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    imageInfo.imageView =
        static_cast<Vulkan::TextureData *>(testTextureData.get())->_getImageView();
    imageInfo.sampler =
        static_cast<Vulkan::TextureData *>(testTextureData.get())->_getSampler();
    ...
    vkUpdateDescriptorSets(graphicCore.getDevice(), 1, &descriptorWrite, 0, nullptr);
  }
  ...
  glm::vec4 clearColor =
      static_cast<Vulkan::RenderPipeline &>(renderer.getRenderPipeline()).getClearColor();
