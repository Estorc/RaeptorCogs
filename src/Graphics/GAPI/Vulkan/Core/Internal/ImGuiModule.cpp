...
void ImGuiModule::createImGuiContext(ImGuiContext *&imGuiContext, Window *window) {
  ...
  // init_info.ColorAttachmentFormat =
  // static_cast<WindowContext*>(window->getContext())->getSurfaceFormat().format;
...
void ImGuiModule::destroyImGuiContext() {
  // Cleanup
...
// static_cast<RaeptorCogs::GAPI::Vulkan::RendererBackend*>(&RaeptorCogs::Renderer().getBackend())->getGraphicCore().getCommandBuffer());
#endif
}
