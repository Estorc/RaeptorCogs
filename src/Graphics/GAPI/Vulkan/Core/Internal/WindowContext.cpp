...
void WindowContext::createSurfaceAndRenderPass() {
  ...
  if (glfwCreateWindowSurface(
          vkRenderer.getGraphicCore().getInstance(),
          this->getWindow()->getGLFWWindow(),
          nullptr,
          &this->getSurface()) != VK_SUCCESS) {
    throw std::runtime_error("failed to create window surface!");
  }

  // Query surface capabilities
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      vkRenderer.getGraphicCore().getPhysicalDevice(),
      this->getSurface(),
      &this->getSurfaceCapabilities());

  // Get surface format
  this->getSurfaceFormat() = findBestSurfaceFormat(
      vkRenderer.getGraphicCore().getPhysicalDevice(), this->getSurface());
  if (vkRenderer.getGraphicCore().getRenderPass() == VK_NULL_HANDLE)
    vkRenderer.getGraphicCore().createRenderPass(this->getSurfaceFormat().format);
}
...
void WindowContext::createSwapchainImages() {
  ...
  this->getSwapExtent() = {
    static_cast<uint32_t>(this->getWindow()->getWidth()),
    static_cast<uint32_t>(this->getWindow()->getHeight())
  };
  ...
  vkCreateSwapchainKHR(
      vkRenderer.getGraphicCore().getDevice(),
      &swapchainInfo,
      nullptr,
      &this->getSwapchain());
}
...
void WindowContext::createImageViews() {
  ...
  if (vkCreateImageView(
          ...
          &viewInfo,
          nullptr,
          &this->getDepthImageView()) != VK_SUCCESS) {
...
void WindowContext::createFramebuffers() {
  ...
  for (size_t i = 0; i < this->getSwapchainImageViews().size(); i++) {
    std::vector<VkImageView> attachments = {
      this->getSwapchainImageViews()[i], this->getDepthImageView()
    };
...
void WindowContext::createCommandBuffers() {
  ...
  vkAllocateCommandBuffers(
      vkRenderer.getGraphicCore().getDevice(), &cmdAllocInfo, cmdBuffers.data());
...
void WindowContext::createFencesAndSemaphores() {
  ...
  fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; // start signaled so first frame doesn't
                                                  // hang
  ...
  for (size_t i = 0; i < this->getMaxFramesInFlight(); ++i) {
    if (vkCreateSemaphore(
            ...
            &semaphoreInfo,
            nullptr,
            &this->getImageAvailableSemaphore(i)) != VK_SUCCESS ||
        vkCreateSemaphore(
            vkRenderer.getGraphicCore().getDevice(),
            &semaphoreInfo,
            nullptr,
            &this->getRenderFinishedSemaphore(i)) != VK_SUCCESS ||
        vkCreateFence(
            vkRenderer.getGraphicCore().getDevice(),
            &fenceInfo,
            nullptr,
            &this->getInFlightFence(i)) != VK_SUCCESS) {
...
void WindowContext::createDescriptorPools() {
  ...
  for (size_t i = 0; i < this->getMaxFramesInFlight(); ++i) {
    ...
    if (vkCreateDescriptorPool(
            ...
            &poolInfo,
            nullptr,
            &this->getDescriptorPools()[i]) != VK_SUCCESS) {
...
void WindowContext::recreateSwapchain(Window *window) {
  ...
  if (window->getWidth() != this->swapExtent.width ||
      window->getHeight() != this->swapExtent.height) {
    std::cout
        << "Recreating swapchain for window resize to " << window->getWidth() << "x"
...
WindowContext::~WindowContext() {
  RendererBackend &vkRenderer =
      static_cast<RendererBackend &>(RaeptorCogs::Renderer().getBackend());
  this->destroySwapchainResources();
  vkDestroyRenderPass(
      vkRenderer.getGraphicCore().getDevice(),
      vkRenderer.getGraphicCore().getRenderPass(),
      nullptr);
  vkFreeCommandBuffers(
      vkRenderer.getGraphicCore().getDevice(),
      vkRenderer.getGraphicCore().getCommandPool(),
      static_cast<uint32_t>(this->getCommandBuffers().size()),
      this->getCommandBuffers().data());
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    vkDestroyDescriptorPool(
        vkRenderer.getGraphicCore().getDevice(), getDescriptorPools()[i], nullptr);
    vkDestroySemaphore(
        vkRenderer.getGraphicCore().getDevice(), getRenderFinishedSemaphore(i), nullptr);
    vkDestroySemaphore(
        vkRenderer.getGraphicCore().getDevice(), getImageAvailableSemaphore(i), nullptr);
...
void WindowContext::destroySwapchainResources() {
  ...
  vkDestroyImageView(
      vkRenderer.getGraphicCore().getDevice(), this->depthImageView, nullptr);
  ...
  for (auto imageView : swapchainImageViews) {
    vkDestroyImageView(vkRenderer.getGraphicCore().getDevice(), imageView, nullptr);
  }
  ...
  vkDestroySwapchainKHR(
      vkRenderer.getGraphicCore().getDevice(), this->swapchain, nullptr);
