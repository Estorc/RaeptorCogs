...
bool Graphic2D::computeInstanceData(
    GAPI::Common::InstanceAllocator &, ComputeInstanceDataMode) {
  throw std::runtime_error("Graphic2D::computeInstanceData must be overridden in derived "
                           "classes.");
}
...
void Graphic2D::setParent(Node *parent) {
  ...
  this->setReadingMaskID(
      parent && parent->isInstanceOf<Graphic2D>()
          ? (static_cast<Graphic2D *>(parent)->getWritingMaskID()
                 ? static_cast<Graphic2D *>(parent)->getWritingMaskID()
                 : static_cast<Graphic2D *>(parent)->getReadingMaskID())
          : 0,
      true);
}

GAPI::Common::GraphicBatchHandler &Graphic2D::getBatchHandler() {
  return RaeptorCogs::Renderer().getBackend().getRenderPipeline().getBatchHandlerAt(
      this->batchHandlerCursor);
}
...
void RenderableGraphic2D::rebuildGlobalColor() {
  ...
  if (this->getParent() && this->getParent()->isInstanceOf<RenderableGraphic2D>()) {
    globalColor =
        (((static_cast<RenderableGraphic2D *>(this->getParent()))->getGlobalColor()) *
         color);
  } else {
    globalColor = color;
  }
...
void TransformableGraphic2D::rebuildGlobalMatrix() {
  ...
  if (this->getParent() && this->getParent()->isInstanceOf<TransformableGraphic2D>()) {
    TransformableGraphic2D *parent =
        static_cast<TransformableGraphic2D *>(this->getParent());
    glm::mat4 parentMatrix = parent->getModelMatrix();
    parentMatrix = glm::translate(parentMatrix, glm::vec3(parent->getAnchor(), 0.0f));
    parentMatrix =
        glm::scale(parentMatrix, glm::vec3(glm::vec2(1.0f) / parent->getSize(), 1.0f));
