...
Window::Window(
    int width, int height, const std::string &title, GLFWmonitor *monitor,
    GLFWwindow *share, RaeptorCogs::GAPI::Common::WindowContext *context,
    WindowFlags flags)
  ...
  if (context) context->initialize(this);
  Renderer().getBackend().getImGuiModule().createImGuiContext(this->imGuiContext, this);

#ifdef __EMSCRIPTEN__
  emscripten_set_element_css_size("#canvas", frameWidth, frameHeight);
  emscripten_set_canvas_element_size("#canvas", width, height);
  emscripten_set_resize_callback(
      EMSCRIPTEN_EVENT_TARGET_WINDOW, this, EM_TRUE, resize_callback);
#endif
...
bool Window::isMouseInWindow() const {
  ...
  return (mousePos.x >= 0 && mousePos.x < windowSize.x && mousePos.y >= 0 &&
