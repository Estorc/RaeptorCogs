...
void Camera2D::rebuildProjectionMatrix() {
  ...
  projectionMatrix =
      glm::ortho(-1.0f * zoom, 1.0f * zoom, -1.0f * zoom, 1.0f * zoom, -1000.0f, 1000.0f);
...
void Camera2D::update(GAPI::Common::RenderPipeline &pipeline) {
  ...
  frameData.projectionMatrix         = glm::scale(
      this->getProjectionMatrix(),
      glm::vec3(
          2.0f / static_cast<float>(frameData.width),
          -2.0f / static_cast<float>(frameData.height),
          1.0f));
}
