...
#else
    std::cerr
        << "Error: Vulkan backend selected but Vulkan support is not compiled in."
        << std::endl;
    exit(EXIT_FAILURE);
#endif
    break;
...
void Renderer::add(Graphic2D &graphic) {
  ...
  GAPI::Common::RenderList &renderList =
      key.writingMask
          ? backend.getRenderPipeline().getMaskRenderList()
          : backend.getRenderPipeline().getRenderList();
  GAPI::Common::GraphicBatchHandler &batchHandler = renderList.createHandler(
      key, &graphic, backend.getGraphicCore().getInstanceAllocator());

  backend.getGraphicCore().getInstanceUploader().markDynamicDataDirty(
      batchHandler.dynamicDataCursor, batchHandler.dynamicDataSize);
  backend.getGraphicCore().getInstanceUploader().markStaticDataDirty(
      batchHandler.staticDataCursor, 1);
}
...
void Renderer::remove(Component &component) {
  ...
  componentBuffer.erase(
      std::remove(componentBuffer.begin(), componentBuffer.end(), &component),
      componentBuffer.end());
  if (RaeptorCogs::IsExiting()) {
    return;
  }
