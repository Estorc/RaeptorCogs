...
bool Sprite2D::computeInstanceData(
  ...
  if (this->isDataDirty() || mode == ComputeInstanceDataMode::REBUILD_TEXTURE ||
      mode == ComputeInstanceDataMode::FORCE_REBUILD) {

    // Static instance data

    staticDataBuffer.model = this->getModelMatrix();
    staticDataBuffer.uvRect =
        texture ? texture->getUVRect() : glm::vec4(0.0f, 0.0f, 1.0f, 1.0f);
    staticDataBuffer.type =
        this->isVisible()
            ? RendererMode::RENDERER_MODE_2D_SPRITE
